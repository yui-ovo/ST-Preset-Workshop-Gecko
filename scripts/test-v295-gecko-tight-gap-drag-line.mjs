...
assert.ok(tuner.includes("'pmm-layout-item-gap-tight'"), '缺少 Gecko 小间距落点线状态');
...
assert.ok(source.includes('V2.95 Gecko 已加载：条目间距小于 3px 时拖拽落点线自动切换到卡片内侧'), '缺少 v2.95 Gecko 运行标记');

console.log('v2.95 Gecko 小间距落点线回归通过：正常间距保留外部线，小于 3px 自动改用内部主题色边线。');
